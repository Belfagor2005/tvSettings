...
TMPPATH=/tmp/tvSettings-install
FILEPATH=/tmp/tvSettings-main.tar.gz

echo "Starting tvSettings installation..."

# Determine plugin path based on architecture
if [ ! -d /usr/lib64 ]; then
    PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/tvSettings
else
    PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/tvSettings
fi
...
cleanup() {
    ...
    [ -d "/tmp/tvSettings-main" ] && rm -rf "/tmp/tvSettings-main"
}
...
echo "⬇️ Downloading tvSettings..."
wget --no-check-certificate 'https://github.com/Belfagor2005/tvSettings/archive/refs/heads/main.tar.gz' -O "$FILEPATH"
if [ $? -ne 0 ]; then
    echo "❌ Failed to download tvSettings package!"
    cleanup
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to extract tvSettings package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/tvSettings-main/usr/lib/enigma2/python/Plugins/Extensions/tvSettings" ]; then
    cp -r "$TMPPATH/tvSettings-main/usr/lib/enigma2/python/Plugins/Extensions/tvSettings"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from standard plugin directory"
elif [ -d "$TMPPATH/tvSettings-main/usr/lib64/enigma2/python/Plugins/Extensions/tvSettings" ]; then
    cp -r "$TMPPATH/tvSettings-main/usr/lib64/enigma2/python/Plugins/Extensions/tvSettings"/* "$PLUGINPATH/" 2>/dev/null
    echo "✅ Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/tvSettings-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/tvSettings-main/usr"/* /usr/ 2>/dev/null
